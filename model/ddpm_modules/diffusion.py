...
class GaussianDiffusion(nn.Module):
    ...
    # @torch.no_grad()
    def generation(self, x_in, continous=False):
        return self.p_sample_loop(x_in, continous)

    # @torch.no_grad()
    def registration(self, x_in, etaRange=np.linspace(1,1,1)):
        x_m = x_in[:, :1]
        x_f = x_in[:, 1:]

        b, c, h, w = x_m.shape
        deform_ = torch.zeros([len(etaRange), c, h, w], device=self.betas.device)

        with torch.no_grad():
            t = torch.full((x_in.shape[0],), 0, device=self.betas.device, dtype=torch.long)

            zero_score = torch.zeros_like(x_m)
            fix_score = self.denoise_fn(torch.cat([x_in, x_f], dim=1), t)
            for ieta, eta in enumerate(etaRange):
                print('---- eta=%.2f registration' % eta)
                score = zero_score * (1-eta) + fix_score * eta
                deform, flow = self.field_fn(torch.cat([x_m, score], dim=1))
                deform_[ieta] = deform[0]
        return deform_, flow
    ...
    def q_sample(self, x_start, t, noise=None):
        ...
        # gama = (r - l) * torch.rand(1, device=x_start.device) + l
        # gama = gama.reshape(t.shape[0], *((1,) * (len(x_shape) - 1)))
        # return (torch.sqrt(gama) * x_start + torch.sqrt(1-gama)* noise)